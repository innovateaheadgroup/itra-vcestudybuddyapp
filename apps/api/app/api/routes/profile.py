...
def profile(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> dict:
    attempt_count = (
        db.scalar(select(func.count(Attempt.id)).where(Attempt.user_id == current_user.id)) or 0
    )
    avg_mastery = (
        db.scalar(select(func.avg(Mastery.mastery_score)).where(Mastery.user_id == current_user.id))
        or 0
    )
    return {
        "id": current_user.id,
